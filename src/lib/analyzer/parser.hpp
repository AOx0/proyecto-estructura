...
inline std::string to_string(const Operator &x) { return to_string(x.variant); }
...
inline std::string to_string(const Keyword &x) { return to_string(x.variant); }

enum TypeE { I8, I16, I32, I64, U8, U16, U32, U64, F64, BOOL, STR };
...
inline std::string to_string(const Type &x) { return to_string(x.variant); }
...
enum SymbolE { COMA, SEMICOLON, ALL, CLOSING_PAR, OPENING_PAR };
...
inline std::string to_string(const Symbol &x) { return to_string(x.variant); }

struct String {
  std::string value;
};

inline std::string to_string(const String &x) { return x.value; }
...
inline std::string to_string(const Name &x) { return x.value; }

struct Unknown {
  std::string value;
};

inline std::string to_string(const Unknown &x) { return x.value; }
...
inline std::string to_string(const Int &x) { return std::to_string(x.value); }

inline std::string to_string(const UInt &x) { return std::to_string(x.value); }
...
using Token = std::variant<Operator, Keyword, Type, Symbol, String, Numbers,
                           Identifier, Unknown, Bool>;

template <typename T> bool is_same_variant(const T &x, const T &y) {
