...
void Parser::make_tokens_explicit(std::string &str) {
  ...
  // Here we rejoin equality operators that were made 'explicit'. Example "=  ="
  // with "=="
...
Parser::validate(const std::string &in) {
  ...
  for (auto &token : tokens) {
    ...
    if (found_in_tokens) {
      ...
      current_type = TokenType::STRING;
               ...
               std::regex_match(token,
                                std::regex("([a-zA-Z_][a-zA-Z0-9_]*)(\\.([a-zA-"
                                           "Z_][a-zA-Z0-9_]*))"))) {
      current_type = TokenType::IDENTIFIER;
    } else {
      return std::make_tuple(
          false, std::make_optional(fmt::format("Invalid token: {}", token)));
    }

    /*
    // It is invalid if we have to operators together
    if (current_type == TokenType::OPERATOR && last_type == TokenType::OPERATOR)
    { return std::make_tuple(false, std::make_optional(fmt::format("Invalid
    found two operators: {} {}", token, token)));
    }

    // It is invalid if we have two identifiers together
    if (current_type == TokenType::IDENTIFIER && last_type ==
    TokenType::IDENTIFIER) { return std::make_tuple(false, std::make_optional(
          fmt::format("Invalid token, found two identifiers: {} {}", token,
    token)));
    }
    ...
    if (current_type == TokenType::TYPE && last_type == TokenType::TYPE) {
      return std::make_tuple(false,
                             std::make_optional(fmt::format("Invalid token,
    found two types: {} {}", token, token)));
    }
    ...
    if (current_type == TokenType::NUMBER && last_type == TokenType::NUMBER) {
      ...
      return std::make_tuple(false,
                             std::make_optional(fmt::format("Invalid token,
    found two numbers: {} {}", token, token)));
    }

    // It is invalid if we have an operator followed by a non value or
    identifier token if (last_type == TokenType::OPERATOR &&
        !(current_type == TokenType::IDENTIFIER || current_type ==
    TokenType::NUMBER || current_type == TokenType::STRING)) { return
    std::make_tuple(false, std::make_optional( fmt::format("Invalid token,
    expected literal (number, string) or identifier after an operator: {}",
    token)));
    }

    // It is invalid if we have an operator preceded by a non value or
    identifier token if (current_type == TokenType::OPERATOR &&
        !(last_type == TokenType::IDENTIFIER || last_type == TokenType::NUMBER
    || last_type == TokenType::STRING)) { return std::make_tuple(false,
    std::make_optional( fmt::format("Invalid token, expected literal (number,
    string) or identifier before an operator: {}", token)));
    }

    // If last is a type and current is not an identifier or a str or a number
    then it is invalid if (last_type == TokenType::TYPE && current_type !=
    TokenType::IDENTIFIER) { return std::make_tuple(false, std::make_optional(
          fmt::format("Invalid token, expected identifier after type: {}",
    token)));
    }*/
