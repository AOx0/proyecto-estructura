...
namespace FileManager {
  ...
  public:
    ...
    static Path
    get_project_dir(std::string const &project_name, std::string const &author_name, std::string const &version) {
      std::string path(get_string(
          rfm_::project_dir((char *) project_name.c_str(), (char *) author_name.c_str(), (char *) version.c_str())));
      return Path{path};
    }
