...
DatabaseTable DatabaseTable::from_vec(const std::vector<std::uint8_t> &in,
  ...
  while (in[i] != '\0') {
    ...
    Layout layout = Serialized::dLayout(&serialized_layout.front(),
...
DatabaseTable::createTable(std::string database, std::string &table_name,
  ...
  if (!table_folder.create_as_dir())
    return cpp::fail(
        fmt::format("Failed while creating directory {}", table_folder.path));

  auto table_info = table_folder / "info.tbl";
  FileManager::write_to_file(table_info.path, t.into_vec());

  auto node =
      t.columns.for_each([&](const KeyValue<std::string, Layout> &value) {
  ...
  if (node != nullptr) {
    return cpp::fail(
        fmt::format("Failed while creating column file {}",
                    (table_folder / (node->value.key + ".col")).path));
  }
